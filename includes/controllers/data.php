...
if ($route == "dashboard") {
	if($isAdmin) {
        ...
        $countAssign=$database->select("labels", "id", [ "name" => "Assign" ]);
        $sumAssets = $database->count("assets", [ "AND" => ["statusid" => $countAssign]]);
        //$sumLicenses = countTable("licenses");
        $countInStock=$database->select("labels", "id", [ "name" => "In Stock" ]);
        $sumLicenses = $database->count("assets", [ "AND" => ["statusid" => $countInStock]]);
		//$sumProjects = countTable("projects");
        $countFaulty=$database->select("labels", "id", [ "name" => "Faulty" ]);
        $sumProjects = $database->count("assets", [ "AND" => ["statusid" => $countFaulty]]);
		//$sumClients = countTable("clients");
		$countOnLoan=$database->select("labels", "id", [ "name" => "On Loan" ]);
        $sumClients = $database->count("assets", [ "AND" => ["statusid" => $countOnLoan]]);
